...
### 📝 指令优化
- `/身体报告` 改名 `/报告`，支持更多别名：`身体报告`、`健康报告`、`建议`
- 更新 README.md 和命令说明文档，保持一致性
...
- 修复无数据显示问题
- 修复日均计算问题
- 睡眠数据展示优化：包含起止时间、评分、深睡/浅睡/REM 分钟数

### 🤖 AI 健康建议
- 新增 AI 健康建议功能，支持个性化健康小贴士生成
- 新增配置项 `health_ai_provider` 和 `health_ai_prompt_template`
- `garmin_detailed_report` 集成 AI 建议，支持自定义 AI 模型
- 默认使用内置规则判断，可配置外部 AI 模型生成建议

### ⚙ 配置项新增
- `health_ai_provider`：健康建议使用的 AI 模型选择
- `health_ai_prompt_template`：健康建议提示词模板，支持 `{health_data}` 占位符
